...
const PrincessTees = () => {
    ...
    const [PrincessTeesssCollections, setPrincessTeesssCollections] = useState([]);
    const PrincessTeesssCollectionsProducts = "http://localhost:9000/v1/products/?sortBy=name&limit=10&page=0&max=500&min=0&categoryId=4&subCategoryId=1";

    useEffect(() => {
        // Fetch the JSON data
        fetch(PrincessTeesssCollectionsProducts)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                setPrincessTeesssCollections(data.data);
                console.log(data);
            })
            .catch(error => {
                console.error('Fetch error:', error);
            });
    }, [PrincessTeesssCollectionsProducts]);
    // eslint-disable-next-line react/prop-types
    const Star = ({ rating }) => {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {PrincessTeesssCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productPrincessTees/${product.id}`}> <img src={product.image} alt={product.name} className="front-img" /></Link>
                        <Link to={`productPrincessTees/${product.id}`}> <img src={product.image} alt="" className="back-img " /></Link>
                            ...
                            onClick={() => {
                                const uniqueId = generateUniqueId();
                                {
                                    addToFav({
                                        ...
                                        size: "S",
                                        color: "Black",
                                        route: "princessTees/productPrincessTees"
                                    });
                                }
                            }}
