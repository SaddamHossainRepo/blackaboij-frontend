...
const Prince = () => {
    ...
    const princeCollectionsProducts = "http://localhost:9000/v1/products?sortBy=name&limit=10&page=0&max=500&min=0&categoryId=3";
    ...
    useEffect(() => {
        ...
        fetch(princeCollectionsProducts)
            ...
            .then(data => {
                setPrinceCollections(data.data);
                console.log(data);
            })
            .catch(error => {
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {princeCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productPrince/${product.id}`}> <img src={product.image} alt={product.name} className="front-img" /></Link>
                        <Link to={`productPrince/${product.id}`}> <img src={product.image} alt="" className="back-img " /></Link>
