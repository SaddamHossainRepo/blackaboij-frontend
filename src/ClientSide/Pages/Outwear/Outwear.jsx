...
const Outwear = () => {
    const [outwearCollections, setOutwearCollections] = useState([]);
    ...
    // const outwearCollectionsProducts = "http://localhost:9000/v1/products/?sortBy=name&limit=10&page=0&max=500&min=0&categoryId=1&subCategoryId=6";
    ...
    useEffect(() => {
        ...
        fetch(menProducts)
            ...
            .then(data => {
                // setOutwearCollections(data.data);
                setOutwearCollections(data);
                console.log(data);
            })
            .catch(error => {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {outwearCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productoutwear/${product.id}`}> <img src={product.img} alt={product.name} className="front-img" /></Link>
                        <Link to={`productoutwear/${product.id}`}> <img src={product.backImg} alt="" className="back-img " /></Link>
