...
const WomensTees = () => {
    ...
    const [womensTeesCollections, setWomensTeesCollections] = useState([]);
    const womensProducts = "./HotSalewomenProducts.json";

    // const womensTeesCollectionsProducts = "http://localhost:9000/v1/products/?sortBy=name&limit=10&page=0&max=500&min=0&categoryId=2&subCategoryId=1";

    useEffect(() => {
        // Fetch the JSON data
        // fetch(womensTeesCollectionsProducts)
        fetch(womensProducts)
            ...
            .then(data => {
                // setWomensTeesCollections(data.data);
                setWomensTeesCollections(data);
                console.log(data);
            })
            .catch(error => {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {womensTeesCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productWomenTees/${product.id}`}> <img src={product.img} alt={product.name} className="front-img" /></Link>
                        <Link to={`productWomenTees/${product.id}`}> <img src={product.backImg} alt="" className="back-img " /></Link>
                            ...
                            onClick={() => {
                                const uniqueId = generateUniqueId();
                                {
                                    addToFav({
                                        ...
                                        size: "S",
                                        color: "Black",
                                        route: "womenTees/productWomenTees"
                                    });
                                }
                            }}
