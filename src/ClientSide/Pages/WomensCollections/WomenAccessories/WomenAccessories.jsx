...
const WomenAccessories = () => {
    ...
    const [womenAccessoriesCollections, setWomenAccessoriesCollections] = useState([]);
    const womenAccessoriesCollectionsProducts = "/Womens/womenAccessories.json";

    useEffect(() => {
        // Fetch the JSON data
        fetch(womenAccessoriesCollectionsProducts)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                setWomenAccessoriesCollections(data.data);
                console.log(data);
            })
            .catch(error => {
                console.error('Fetch error:', error);
            });
    }, [womenAccessoriesCollectionsProducts]);
    // eslint-disable-next-line react/prop-types
    const Star = ({ rating }) => {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {womenAccessoriesCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productWomenAccessories/${product.id}`}> <img src={product.img} alt={product.productName} className="front-img" /></Link>
                        <Link to={`productWomenAccessories/${product.id}`}> <img src={product.backImg} alt="" className="back-img " /></Link>
                            ...
                            onClick={() => {
                                const uniqueId = generateUniqueId();
                                {
                                    addToFav({
                                        ...
                                        size: "S",
                                        color: "Black",
                                        route: "womenAccessories/productWomenAccessories"
                                    });
                                }
                            }}
