...
const ManCollections = () => {
    ...
    // const menCollectionsProducts = "http://localhost:9000/v1/products?sortBy=name&limit=6&page=0&max=500&min=0&categoryId=1";
    ...
    useEffect(() => {
        ...
        fetch(menProducts)
            ...
            .then(data => {
                // setMenCollections(data.data);
                setMenCollections(data);
                console.log('man collection data',data);
            })
            .catch(error => {
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {menCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productMen/${product.id}`}> <img src={product.img} alt={product.name} className="front-img" /></Link>
                            ...
                            onClick={() => {
                                const uniqueId = generateUniqueId();
                                {
                                    addToFav({
                                        ...
                                        size: "S",
                                        color: "Black",
                                        route: "menCollections/productMen"
                                    });
                                }
                            }}
