...
const HoddiesAndSweeters = () => {
    const [hoddiesAndSweetersCollections, setHoddiesAndSweetersCollections] = useState([]);
    ...
    // const hoddiesAndSweetersCollectionsProducts = "http://localhost:9000/v1/products/?sortBy=name&limit=10&page=0&max=500&min=0&categoryId=1&subCategoryId=2";
    ...
    useEffect(() => {
        ...
        fetch(menProducts)
            ...
            .then(data => {
                // setHoddiesAndSweetersCollections(data.data);
                setHoddiesAndSweetersCollections(data);
                console.log('hoodies', data);
            })
            .catch(error => {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {hoddiesAndSweetersCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productHoodies/${product.id}`}> <img src={product.img} alt={product.name} className="front-img" /></Link>
                        <Link to={`productHoodies/${product.id}`}> <img src={product.backImg} alt="" className="back-img " /></Link>
                            ...
                            onClick={() => {
                                const uniqueId = generateUniqueId();
                                {
                                    addToFav({
                                        ...
                                        size: "S",
                                        color: "Black",
                                        route: "hoodiesAndSweeters/productHoodies"
                                    });
                                }
                            }}
...
export default HoddiesAndSweeters;
